...
class ChangePasswordView(FormView):
    ...
    def get(self, *args, **kwargs):
        if not self.request.user.is_authenticated:
            return redirect("account:account_password_reset")
    ...
    def send_email(self, email):
        ...
        for user in User.objects.filter(pk__in=email_qs.values("user")):
            ...
            password_reset_url = "{0}://{1}{2}".format(
                protocol,
                current_site.domain,
                reverse("account:account_password_reset_token", kwargs=dict(uidb36=uid, token=token))
            )
            ctx = {
                "user": user,
                "current_site": current_site,
...
class SettingsView(LoginRequiredMixin, FormView):
    ...
    def get_form_class(self):
        ...
        self.account_key = self.request.user.account.account_key
    ...
    def get_initial(self):
        ...
        initial["account_key"] = self.request.user.account.account_key
        return initial
    ...
    def get_context_data(self, **kwargs):
        ...
        redirect_field_name = self.get_redirect_field_name()
        ctx.update({"account_key": self.request.user.account.account_key})
        ctx.update({
            "redirect_field_name": redirect_field_name,
            "redirect_field_value": self.request.POST.get(redirect_field_name, self.request.GET.get(redirect_field_name, "")),
        })
